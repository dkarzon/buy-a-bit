...
export function ProductNewPage() {
  ...
  if (created) {
    return (
      <main className="mx-auto grid min-h-screen max-w-lg place-content-center gap-6 px-6 py-10">
        <Brand />
        <div className="settings-card text-center">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-muted">Use this QR code or share the live product page.</p>
          </div>
  ...
  return (
    <main className="mx-auto max-w-lg px-6 py-10">
      <button className="mb-7 flex items-center gap-2 text-sm text-muted" onClick={() => navigate(-1)}>
        <ArrowLeft size={17} /> Back to dashboard
      </button>
      <Brand />
      <div className="mt-8">
        <h1 className="text-3xl font-semibold">New product</h1>
        <p className="mt-2 text-sm text-muted">Create a product and generate its live QR code.</p>
      </div>
      <form className="mt-7 flex flex-col gap-4" onSubmit={(event) => void submit(event)}>
        ...
        <Button type="submit" disabled={createProduct.isPending}>
          <PackagePlus size={17} /> {createProduct.isPending ? "Creating product…" : "Create product & get QR"}
        </Button>
      </form>
    </main>
  );
